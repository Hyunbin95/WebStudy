	...
	window.onload = function(){
		...
		document.getElementById("code").addEventListener(
				'focusout', function(e){
					...
					//전송받을 데이터 타입은 json
					//데이터를 가져오면 가져온 데이터를 출
					$.ajax({
						url:'codecheck.do',
						data:{'code':document.getElementById("code").value},
						dataType:'json',
						success:function(d){
							//==는 값만 같으면 true ===는 자료형까지 일치
							if(d.result === 'success'){
		...
		//form에서 submit 버튼을 눌렀을 때
		//기본 이벤트를 제거 : 매개변수.preventDefault()
...
<body>
	...
	<form action="create.do" method="post" id="myform">
		...
		<input type="number" name="code" id="code" size="10"
		required="required" />
